...
const VideoNews = () => {
        ...
        .then(response => {
            ...
            setVideoNews(JSON.parse(localStorage.getItem('videoNews')) && JSON.parse(localStorage.getItem('videoNews')).splice(0, 8).map((news, i) => {
                const animIn = () => { // anim mouse in
                    $(`.newsHr #${'id' + i} .img img`).css({'transform': 'scale(1.04)'});
                    $(`.newsHr #${'id' + i}`).css({'boxShadow': '0px 0px 15px 1px #000'});
                    $(`.newsHr #${'id' + i} h3`).css({'backgroundColor': 'rgba(0, 0, 0, 0.7)'}).css({'color': 'rgb(224, 164, 79)'});
                    $(`.newsHr #${'id' + i} .img img`).css({'opacity': '1'});
                }
                const animOut = () => { // anim mouse out
                    $(`.newsHr #${'id' + i} .img img`).css({'transform': 'scale(1)'});
                    $(`.newsHr #${'id' + i}`).css({'boxShadow': '0px 0px 0px 0px #000'});
                    $(`.newsHr #${'id' + i} h3`).css({'backgroundColor': 'rgba(0, 0, 0, 0.1)'}).css({'color': 'rgb(255, 255, 255)'});
                    $(`.newsHr #${'id' + i} .img img`).css({'opacity': '0.8'});
                }
    ...
    return (
        <div className="newsHr" id='videoNews'>
            <section>
