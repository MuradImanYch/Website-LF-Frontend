...
const Results = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
              const response = await axios.get('/results/seriea');
    ...
    return (
        <div className='leagueResults'>
          <Helmet>
                <title>СерияА - результаты матчей - на Legendary Football</title>
                <meta name="description" content="Результаты матчей чемпионата италии (Серии А)." />
                <meta name="keywords" content="серия а, чемпионат италии, итальянский футбол, футбол, ювентус, наполи, аталанта, рома, интер, результаты серии а, прошедшие матчи серии а" />
            </Helmet>
            <div className="logoPageName">
                <LazyLoad offset={800}>
                    <Tippy content='Серия А'><img loading="lazy" src={logo} alt="logo" /></Tippy>
                </LazyLoad>
                <h1 className="pageName">Результаты - Серия А</h1>
            </div>
