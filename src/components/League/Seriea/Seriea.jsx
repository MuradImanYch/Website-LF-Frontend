...
const Seriea = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            await axios.get('/fixtures/seriea')
            .then(response => {
                let filtered = response.data.filter(e => {
                    return e.round === response.data[0].round;
                });
    ...
    return (
        <div id='leagueSeriea'>
            <Helmet>
                <title>Серия А - новости, результаты, расписание матчей, турнирная таблица и много чего - на Legendary Football</title>
                <meta name="description" content="Изучайте последние новости, результаты и турнирную таблицу Серии А на нашем сайте. У нас вы найдете все необходимые материалы о главном итальянском футбольном чемпионате и в целом об итальянском футболе." />
                <meta name="keywords" content="серия а, новости, результаты, турнирная таблица, футбол, итальянский футбол, чемпионат италии, ювентус, интер, милан, аталанта, наполи, список трансферов, бомбардиры, ближайшие матчи, последние результаты, рейтинг в уефа, рейтинг в фифа, список популярных трансферов, видео, блоги" />
            </Helmet>
           <div className="logoPageName">
                <div className="info">
                    <div className='left'>
                        <div>
                            <LazyLoad offset={800}><Tippy trigger={$(window).width() < 1024 ? 'click' : 'mouseenter'} content='Серия А'><img loading="lazy" src={logo} alt="logo" /></Tippy></LazyLoad>
                        </div>
                        <div>
                            <h1 className="pageName">Серия А <span>Сезон: {season}</span></h1>
                            <span>Место проведения: <span>Италия</span></span>
                        </div>
                    </div>
                    <div className="right">
