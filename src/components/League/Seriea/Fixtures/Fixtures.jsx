...
const Fixtures = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
              const response = await axios.get('/fixtures/seriea');
    ...
    return (
        <div className='leagueFixtures'>
          <Helmet>
                <title>Серия А - расписание матчей - на Legendary Football</title>
                <meta name="description" content="Календарь матчей чемпионата италии (Серии А)." />
                <meta name="keywords" content="серия а, чемпионат италии, итальянский футбол, футбол, ювентус, интер, наполи, рома, аталанта, лацио, календарь серии а, расписание серии а" />
            </Helmet>
            <div className="logoPageName">
                <LazyLoad offset={800}>
                    <Tippy content='Серия А'><img loading="lazy" src={logo} alt="logo" /></Tippy>
                </LazyLoad>
                <h1 className="pageName">Календарь - Серия А</h1>
            </div>
            {fixtures && fixtures.length > 0 ? fixtures.map((fixture, index) => (
        <React.Fragment key={'seriea' + index}>
