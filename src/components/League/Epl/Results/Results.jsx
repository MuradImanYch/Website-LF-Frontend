...
const Results = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
              const response = await axios.get('/results/epl');
    ...
    return (
        <div className='leagueResults'>
          <Helmet>
                <title>Английская Премьер Лига (Ла лига) - результаты матчей - на Legendary Football</title>
                <meta name="description" content="Результаты матчей чемпионата англии (АПЛ)." />
                <meta name="keywords" content="апл, чемпионат англии, английский футбол, футбол, манчестер сити, манчестер юнайтед, челси, арсенал, результаты апл, прошедшие матчи апл" />
            </Helmet>
            <div className="logoPageName">
                <LazyLoad offset={800}>
                    <Tippy content='АПЛ'><img loading="lazy" src={logo} alt="logo" /></Tippy>
                </LazyLoad>
                <h1 className="pageName">Результаты - Английская Премьер-Лига</h1>
            </div>
            {results && results.length > 0 ? results.map((fixture, index) => (
        <React.Fragment key={'laliga' + index}>
