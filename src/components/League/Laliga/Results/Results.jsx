  ...
  function convertGermanToclientTime(germanTime) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
              const response = await axios.get('/results/laliga');
    ...
    return (
        <div className='leagueResults'>
          <Helmet>
                <title>Ла лига - результаты матчей - на Legendary Football</title>
                <meta name="description" content="Результаты матчей чемпионата испании (Ла лиги)." />
                <meta name="keywords" content="ла лига, чемпионат испании, испанский футбол, футбол, реал мадрид, атлетико мадрид, барселона, севилья, результаты ла лиги, прошедшие матчи ла лиги" />
            </Helmet>
            <div className="logoPageName">
                <LazyLoad offset={800}>
                    <Tippy trigger={$(window).width() < 1024 ? 'click' : 'mouseenter'} content='Ла Лига'><img loading="lazy" src={logo} alt="logo" /></Tippy>
                </LazyLoad>
                <h1 className="pageName">Результаты - Ла Лига</h1>
            </div>
