...
const Results = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
          const response = await axios.get('/results/ucl');
    ...
    return (
        <div className='leagueResults'>
          <Helmet>
                <title>Лига Чемпионов (ЛЧ) - Результаты матчей</title>
                <meta name="description" content="Результаты матчей Лиги Чемпионов (ЛЧ)." />
                <meta name="keywords" content="лига чемпионов результаты, лч результаты, результаты лч, лига чемпионов матчи, европейский футбол результаты, итоги лч, результаты лиги чемпионов, реал мадрид результаты, атлетико мадрид результаты, бавария результаты, манчестер сити результаты, прошедшие матчи лч" />
            </Helmet>
            <div className="logoPageName">
                <LazyLoad offset={800}>
                    <Tippy trigger={$(window).width() < 1024 ? 'click' : 'mouseenter'} content='ЛЧ'><img loading="lazy" src={logo} alt="logo" /></Tippy>
                </LazyLoad>
                <h1 className="pageName" style={localStorage.getItem('darkTheme') === 'true' ? {color: '#fff'} : null}>Результаты - Лига Чемпионов</h1>
            </div>
