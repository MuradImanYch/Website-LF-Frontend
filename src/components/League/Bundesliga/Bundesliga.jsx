...
const Bundesliga = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            await axios.get('/fixtures/bundesliga')
            .then(response => {
                let filtered = response.data.filter(e => {
                    return e.round === response.data[0].round;
                });
    ...
    return (
        <div>
            <Helmet>
                <title>Бундеслига - новости, результаты, расписание матчей, турнирная таблица и много чего - на Legendary Football</title>
                <meta name="description" content="Изучайте последние новости, результаты и турнирную таблицу Бундеслиги на нашем сайте. У нас вы найдете все необходимые материалы о главном немецком футбольном чемпионате и в целом о немецком футболе." />
                <meta name="keywords" content="бундеслига, новости, результаты, турнирная таблица, футбол, немецкий футбол, чемпионат германии, бавария, боррусия дортмунд, лейпциг, айнтрахт франкфурт, серебрянная салатница, бомбардиры, ближайшие матчи, последние результаты, рейтинг в уефа, рейтинг в фифа, список популярных трансферов, видео, блоги" />
            </Helmet>
            <div className="logoPageName">
                <div className="info">
                    <div className='left'>
                        <div>
                            <LazyLoad offset={800}><Tippy trigger={$(window).width() < 1024 ? 'click' : 'mouseenter'} content='Бундеслига'><img loading="lazy" src={logo} alt="logo" /></Tippy></LazyLoad>
                        </div>
                        <div>
                            <h1 className="pageName">Бундеслига <span>Сезон: {season}</span></h1>
                            <span>Место проведения: <span>Германия</span></span>
                        </div>
                    </div>
                    <div className="right">
