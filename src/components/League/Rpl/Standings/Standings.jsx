            ...
            {standings && standings.length > 0 ? <div className="wrap">
                <div className='col'>
                    ...
                    {standings && standings.map((e, i) => {
                        return <Tippy trigger={$(window).width() < 1024 ? 'click' : 'mouseenter'} key={'place' + i} content={e.description && e.description.includes('Лиги') ? e.description + ' (?)*' : e.description}><span className={`place ${e.descrLat && e.descrLat.includes('Ligi') ? null : e.descrLat}`}>{e.place}</span></Tippy>
                    })}
                </div>
                <div className='col'>
