  ...
  function convertGermanToclientTime(germanTime) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
              const response = await axios.get('/fixtures/rpl');
    ...
    return (
        <div className='leagueFixtures'>
          <Helmet>
                <title>Российская Премьер Лига (РПЛ) - расписание матчей - на Legendary Football</title>
                <meta name="description" content="Календарь матчей чемпионата россии (РПЛ)." />
                <meta name="keywords" content="рпл, чемпионат россии, российский футбол, футбол, зенит, цска, краснодар, спартак москва, календарь рпл, расписание рпл" />
            </Helmet>
            <div className="logoPageName">
                <LazyLoad offset={800}>
                    <Tippy trigger={$(window).width() < 1024 ? 'click' : 'mouseenter'} content='РПЛ'><img loading="lazy" src={logo} alt="logo" /></Tippy>
                </LazyLoad>
                <h1 className="pageName">Календарь - Российская Премьер-Лига</h1>
            </div>
            {fixtures && fixtures.length > 0 ? fixtures.map((fixture, index) => (
        <React.Fragment key={'rpl' + index}>
