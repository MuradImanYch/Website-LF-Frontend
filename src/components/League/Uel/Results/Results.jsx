  ...
  useEffect(() => {
    ...
    return (
        <div className='leagueResults'>
          <Helmet>
                <title>Лига Европы (ЛЕ) - Результаты матчей</title>
                <meta name="description" content="Результаты матчей Лиги Европы (ЛЕ)." />
                <meta name="keywords" content="лига европы результаты, ле результаты, результаты ле, лига европы матчи, европейский футбол результаты, итоги ле, результаты лиги европы, ливерпуль результаты, айнтрахт результаты, карабах результаты, рома результаты, прошедшие матчи ле" />
            </Helmet>
            <div className="logoPageName">
                <LazyLoad offset={800}>
                    <Tippy trigger={$(window).width() < 1024 ? 'click' : 'mouseenter'} content='ЛЕ'><img loading="lazy" src={logo} alt="logo" /></Tippy>
                </LazyLoad>
                <h1 style={localStorage.getItem('darkTheme') === 'true' ? {color: '#fff'} : null} className="pageName">Результаты - Лига Европы</h1>
            </div>
