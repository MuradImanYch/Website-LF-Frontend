...
const Uel = () => {
                ...
                setUefaRank(response.data && response.data.splice(1, 5).map((e, i) => {
                    return <div className='inner' key={'uefaRank' + i}>
                                ...
                                <div className='points'>
                                    <span style={localStorage.getItem('darkTheme') === 'true' ? {color: '#fff'} : null} className='points'>{e.total}</span>
                                </div>
                    </div>
                }));
            })
    ...
    return (
        <div id='leagueUel'>
            <Helmet>
                <title>Лига Европы (ЛЕ) - новости, результаты, расписание матчей, турнирная таблица</title>
                <meta name="description" content="Читайте последние новости, результаты и турнирную таблицу ЛЕ на нашем сайте." />
                <meta name="keywords" content="ле, новости ле, результаты ле, турнирная таблица ле, европейский футбол новости, лига европы бомбардиры, айнтрахт, ливерпуль, манчестер юнайтед, карабах, список бомбардиров ле, ближайшие матчи ле, последние результаты лига европы" />
            </Helmet>
            <div className="logoPageName">
                <div className="info">
                    <div className='left'>
                        <div>
                            <LazyLoad offset={800}><Tippy trigger={$(window).width() < 1024 ? 'click' : 'mouseenter'} content='ЛЕ'><img loading="lazy" src={logo} alt="logo" /></Tippy></LazyLoad>
                        </div>
                        <div>
                            <h1 style={localStorage.getItem('darkTheme') === 'true' ? {color: '#fff'} : null} className="pageName">Лига Европы <span style={localStorage.getItem('darkTheme') === 'true' ? {color: '#fff'} : null}>Сезон: {season}</span></h1>
            ...
            <div className="standingsTopScores">
                <div className="standingsWrap">
                    ...
                    <div className="table5xn standings">
                        ...
                        <Link style={localStorage.getItem('darkTheme') === 'true' ? {color: '#fff'} : null} to="/league/uel/standings">Подробнее</Link>
                    </div>
                </div>
                ...
                <div className="topScoresWrap">
                    ...
                    <div className="table6xn topScores">
                        ...
                        <Link style={localStorage.getItem('darkTheme') === 'true' ? {color: '#fff'} : null} to="/league/uel/topscores">Подробнее</Link>
                    </div>
                </div>
            </div>
